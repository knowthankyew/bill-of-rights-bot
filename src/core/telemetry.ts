 ...
 * 2. Raw agreement bodies, subscription terms, or consumer PII payloads are strictly forbidden from attributes.
...
export function getPrivacyClaims(report: PrivacyAuditReport): PrivacyClaims {
  if (report.isLocalOnlyHonest) {
    return {
      ...
      disclaimerExecutionText: 'All subscription audits execute 100% locally in your browser with zero remote network transmission.',
      footerTitle: '100% Local Air-Gapped Subscription Agreement Reality Engine.',
      footerSubtext: 'Zero Telemetry • Zero Remote Agreement Egress • FTC Click-to-Cancel & State ARL Reality Engine',
  ...
  return {
    ...
    footerTitle: 'Enterprise Subscription Agreement Reality Engine (OTLP Telemetry Active).',
    footerSubtext: 'Enterprise Telemetry Mode • Operational Metadata Export Active • Agreements Air-Gapped',
      ...
      await fetch(this.config.otlpEndpoint, {
        ...
        body: JSON.stringify({
          resourceSpans: [
            {
              resource: {
                attributes: [{ key: 'service.name', value: { stringValue: 'bill-of-rights-bot' } }],
              },
              scopeSpans: [
                {
                  spans: [
